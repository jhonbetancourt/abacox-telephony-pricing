...
public class InventoryOwnerController {
    ...
    public InventoryOwnerDto activate(@PathVariable("id") Long id, @Valid @RequestBody ActivationDto activationDto) {
        return modelConverter.map(inventoryOwnerService.changeActivation(id, activationDto.getActive()), InventoryOwnerDto.class);
    }

    @GetMapping(value = "/export/excel", produces = MediaType.APPLICATION_OCTET_STREAM_VALUE)
    public ResponseEntity<Resource> exportExcel(@Parameter(hidden = true) @Filter Specification<InventoryOwner> spec,
            @Parameter(hidden = true) Pageable pageable,
            @ParameterObject FilterRequest filterRequest,
            @ParameterObject ExcelRequest excelRequest) {
        ByteArrayResource resource = inventoryOwnerService.exportExcel(spec, pageable, excelRequest.toExcelGeneratorBuilder());
