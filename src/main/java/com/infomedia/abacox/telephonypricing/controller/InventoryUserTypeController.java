...
public class InventoryUserTypeController {
    ...
    public InventoryUserTypeDto activate(@PathVariable("id") Long id, @Valid @RequestBody ActivationDto activationDto) {
        return modelConverter.map(inventoryUserTypeService.changeActivation(id, activationDto.getActive()), InventoryUserTypeDto.class);
    }

    @GetMapping(value = "/export/excel", produces = MediaType.APPLICATION_OCTET_STREAM_VALUE)
    public ResponseEntity<Resource> exportExcel(@Parameter(hidden = true) @Filter Specification<InventoryUserType> spec,
            @Parameter(hidden = true) Pageable pageable,
            @ParameterObject FilterRequest filterRequest,
            @ParameterObject ExcelRequest excelRequest) {
        ByteArrayResource resource = inventoryUserTypeService.exportExcel(spec, pageable, excelRequest.toExcelGeneratorBuilder());
