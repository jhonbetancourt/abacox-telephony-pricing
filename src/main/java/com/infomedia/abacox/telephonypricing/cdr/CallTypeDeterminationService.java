...
public class CallTypeDeterminationService {
    ...
    private void processIncomingLogic(CdrData cdrData, CommunicationLocation commLocation, ExtensionLimits limits) {
        ...
        // cdrData.callingPartyNumber = our extension
        // cdrData.finalCalledPartyNumber = external number
        String externalCallerIdRaw = cdrData.getFinalCalledPartyNumber();
        String ourExtension = cdrData.getCallingPartyNumber();
        cdrData.setEffectiveDestinationNumber(ourExtension); // Our extension is the "destination" of this leg
        log.debug("Incoming call. External Caller ID (raw): {}, Our Extension (now callingParty): {}", externalCallerIdRaw, ourExtension);

        String externalCallerIdProcessed = externalCallerIdRaw;

        Optional<String> pbxTransformedCaller = pbxSpecialRuleLookupService.applyPbxSpecialRule(
                externalCallerIdProcessed, commLocation.getDirectory(), 1
        );
        if (pbxTransformedCaller.isPresent()) {
            log.debug("External Caller ID '{}' transformed by PBX incoming rule to '{}'", externalCallerIdProcessed, pbxTransformedCaller.get());
            cdrData.setOriginalCallerIdBeforePbxIncoming(externalCallerIdProcessed);
            externalCallerIdProcessed = pbxTransformedCaller.get();
            cdrData.setPbxSpecialRuleAppliedInfo("PBX Incoming Rule: " + cdrData.getFinalCalledPartyNumber() + " -> " + externalCallerIdProcessed);
        }

        TransformationResult transformedIncoming = phoneNumberTransformationService.transformIncomingNumberCME(
                externalCallerIdProcessed, commLocation.getIndicator().getOriginCountryId()
        );
        if (transformedIncoming.isTransformed()) {
            log.debug("External Caller ID '{}' transformed by CME rule to '{}'", externalCallerIdProcessed, transformedIncoming.getTransformedNumber());
            cdrData.setOriginalCallerIdBeforeCMETransform(externalCallerIdProcessed);
            externalCallerIdProcessed = transformedIncoming.getTransformedNumber();
        ...
        IncomingCallOriginInfo originInfo = callOriginDeterminationService.determineIncomingCallOrigin(
            externalCallerIdProcessed, commLocation
        );
        ...
        // Update finalCalledPartyNumber (which holds the external number) if determineIncomingCallOrigin transformed it
        if (!Objects.equals(externalCallerIdProcessed, originInfo.getEffectiveNumber())) {
            log.debug("Incoming call's external number (finalCalledPartyNumber) updated after origin determination from '{}' to '{}'",
                    externalCallerIdProcessed, originInfo.getEffectiveNumber());
            cdrData.setFinalCalledPartyNumber(originInfo.getEffectiveNumber());
            // EffectiveDestinationNumber is still our extension for this leg.
        }
    ...
    private void processInternalCallLogic(CdrData cdrData, CommunicationLocation commLocation, ExtensionLimits limits, boolean pbxSpecialRuleAppliedRecursively) {
        ...
        boolean stripOnlyIfPrefixMatchesAndFound = true;
        if (pbxSpecialRuleAppliedRecursively && commLocation.getPbxPrefix() != null && !commLocation.getPbxPrefix().isEmpty()) {
            ...
            stripOnlyIfPrefixMatchesAndFound = false;
        }
    ...
    private InternalCallTypeInfo determineSpecificInternalCallType(CdrData cdrData, CommunicationLocation currentCommLocation, ExtensionLimits limits) {
        ...
        if (destEmpOpt.isEmpty()) {
            ...
            Long destOfficeId = destSubdivision != null ? destSubdivision.getId() : null; // Assuming Subdivision ID is office ID
