...
public class CdrConfigService {
    ...
    public static final String DEFAULT_TARGET_DATABASE_TIMEZONE_ID = "America/Bogota"; // Added
    ...
    private String getParameter(String paramName, String defaultValue, Long plantTypeId, String clientBdName) {
        ...
        // In a real system, this would query a configuration table.
        // For now, return hardcoded defaults based on PHP analysis or new additions.
        if ("TARGET_DATABASE_TIMEZONE".equals(paramName)) return DEFAULT_TARGET_DATABASE_TIMEZONE_ID; // Added
    ...
    // Added method to get the target ZoneId
    public ZoneId getTargetDatabaseZoneId() {
        String zoneIdStr = getParameter("TARGET_DATABASE_TIMEZONE", DEFAULT_TARGET_DATABASE_TIMEZONE_ID, null, null);
        try {
            return ZoneId.of(zoneIdStr);
        } catch (Exception e) {
            log.error("Invalid ZoneId configured: '{}'. Defaulting to UTC.", zoneIdStr, e);
            return ZoneId.of("UTC"); // Fallback to UTC if configured one is invalid
        }
    }
