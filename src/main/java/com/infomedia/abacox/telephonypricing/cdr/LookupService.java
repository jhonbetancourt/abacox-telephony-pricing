...
public class LookupService {
    ...
    private final CdrProcessingConfig configService; // Added for canonical operator lookup

    // Constructor injection for CdrProcessingConfig
    ...
    public Optional<Employee> findEmployeeByExtensionOrAuthCode(String extension, String authCode, Long commLocationId) {
        ...
        if (commLocationId != null && commLocationId > 0) {
            ...
            log.trace("CommLocationId is null or 0 for employee lookup. Searching globally or based on other criteria.");
        }
        ...
        for (int ndcLength = maxNdcLength; ndcLength >= minNdcLength; ndcLength--) {
            ...
            if (ndcStr.matches("\\d*") && subscriberNumberStr.matches("\\d+")) {
                ...
                if (isPrefixBandOk) {
                    ...
                    sqlBuilder.append("WHERE b.origin_indicator_id IN (0, :originCommLocationIndicatorId) "); // 0 for global origin band
                } else {
                ...
                // PHP: $filtro_origen for non-international/satellite
                ...
                } else if (currentPrefixId != null) { // For the operator subquery
                    query.setParameter("currentPrefixId", currentPrefixId);
                }
    ...
    public Map<String, Integer> findNdcMinMaxLength(Long telephonyTypeId, Long originCountryId) {
        ...
        sqlBuilder.append("  AND i.telephony_type_id = :telephonyTypeId ");
        sqlBuilder.append("  AND i.origin_country_id IN (0, :originCountryId) ");
        sqlBuilder.append("  AND s.ndc IS NOT NULL AND s.ndc > 0 ");
    ...
    public Optional<String> findCompanyForNationalSeries(int ndc, long subscriberNumber, Long originCountryId) {
        ...
        Optional<Operator> canonicalNationalOperatorOpt = configService.getOperatorInternal(nationalTelephonyTypeId, originCountryId);
        Long canonicalNationalOperatorId = canonicalNationalOperatorOpt.map(Operator::getId).orElse(null);
        ...
        if (canonicalNationalOperatorId != null) {
            sqlBuilder.append("  AND (i.operator_id = 0 OR i.operator_id IS NULL OR i.operator_id = :canonicalNationalOperatorId) ");
        } else {
            sqlBuilder.append("  AND (i.operator_id = 0 OR i.operator_id IS NULL) "); // Fallback if no canonical operator
            log.warn("No canonical national operator found for country {}. Company lookup for national series might be less specific.", originCountryId);
        }
        ...
        query.setParameter("originCountryId", originCountryId);
        if (canonicalNationalOperatorId != null) {
            query.setParameter("canonicalNationalOperatorId", canonicalNationalOperatorId);
        }
    ...
    public Optional<Trunk> findTrunkByCode(String trunkCode, Long commLocationId) {
        ...
        String sql = "SELECT t.* FROM trunk t " +
                ...
                "  AND t.comm_location_id IN (0, :commLocationId) " +
                "ORDER BY t.comm_location_id DESC " +
                "LIMIT 1";
    ...
    public Optional<TrunkRule> findTrunkRule(String trunkCode, Long telephonyTypeId, Long indicatorId, Long originCommLocationIndicatorId) {
        ...
        log.debug("Finding trunk rule for trunkCode: '{}', ttId: {}, indIdStr: {}, effectiveOriginIndId: {}",
                trunkCode, telephonyTypeId, indicatorIdStr, effectiveOriginIndicatorId);
        ...
        sqlBuilder.append("  AND (tr.trunk_id = 0 OR tr.trunk_id IS NULL OR (t.name = :trunkCode)) ");
        ...
        sqlBuilder.append("ORDER BY CASE WHEN tr.trunk_id IS NOT NULL AND tr.trunk_id != 0 THEN 0 ELSE 1 END, ");
        sqlBuilder.append("         CASE WHEN tr.origin_indicator_id IS NOT NULL AND tr.origin_indicator_id != 0 THEN 0 ELSE 1 END, ");
        if (indicatorIdStr != null) {
        ...
        try {
            ...
            log.trace("No matching TrunkRule found for trunk: '{}', ttId: {}, indId: {}, originIndId: {}", trunkCode, telephonyTypeId, indicatorIdStr, effectiveOriginIndicatorId);
            return Optional.empty();
        } catch (Exception e) {
            log.error("Error finding trunk rule for trunk: '{}', ttId: {}, indId: {}, originIndId: {}", trunkCode, telephonyTypeId, indicatorIdStr, effectiveOriginIndicatorId, e);
            return Optional.empty();
        }
    }
    ...
    public List<PbxSpecialRule> findPbxSpecialRuleCandidates(Long commLocationId, int direction) {
        ...
        String sql = "SELECT p.* FROM pbx_special_rule p " +
                ...
                "ORDER BY CASE WHEN p.comm_location_id IS NOT NULL AND p.comm_location_id != 0 THEN 0 ELSE 1 END, LENGTH(p.search_pattern) DESC";
    ...
    public List<Map<String, Object>> findSpecialRateValues(Long telephonyTypeId, Long operatorId, Long bandId, Long originCommLocationIndicatorId, LocalDateTime callDateTime) {
        ...
        StringBuilder sqlBuilder = new StringBuilder();
        sqlBuilder.append("SELECT srv.id as srv_id, srv.name as srv_name, srv.rate_value as srv_rate_value, srv.value_type as srv_value_type, ");
        sqlBuilder.append("       srv.includes_vat as srv_includes_vat, srv.hours_specification as srv_hours_spec, ");
        ...
        sqlBuilder.append(") ");
        sqlBuilder.append("ORDER BY CASE WHEN srv.origin_indicator_id IS NOT NULL AND srv.origin_indicator_id != 0 THEN 0 ELSE 1 END, ");
        sqlBuilder.append("         CASE WHEN srv.telephony_type_id IS NOT NULL AND srv.telephony_type_id != 0 THEN 0 ELSE 1 END, ");
        sqlBuilder.append("         CASE WHEN srv.operator_id IS NOT NULL AND srv.operator_id != 0 THEN 0 ELSE 1 END, ");
        sqlBuilder.append("         CASE WHEN srv.band_id IS NOT NULL AND srv.band_id != 0 THEN 0 ELSE 1 END");
    ...
    public Map<String, Integer> findTelephonyTypeMinMaxConfig(Long telephonyTypeId, Long originCountryId) {
        ...
        try {
            ...
            log.trace("No active TelephonyTypeConfig found for type {}, country {}. Using defaults.", telephonyTypeId, originCountryId);
        } catch (Exception e) {
    ...
    public Map<String, Integer> findExtensionMinMaxLength(Long commLocationId) {
        ...
        sqlEmployee.append("  AND e.extension ~ '^[0-9]+$' "); // PHP: not like '0%' and not like '%-%'
        sqlEmployee.append("  AND e.extension NOT LIKE '0%' ");
        sqlEmployee.append("  AND e.extension NOT LIKE '%-%' ");
    ...
    public boolean isPrefixUniqueToOperator(String prefixCode, Long telephonyTypeId, Long originCountryId) {
        if (!StringUtils.hasText(prefixCode) || telephonyTypeId == null || originCountryId == null) {
            return false; // Or throw an error, but false is safer for this check
        }
